...
public class D21P2 {
	...
	public static void main(String[] args) {
		...
		//Go through 18 iterations
		boolean[] current = enhance(start); //First iteration is special
		ArrayList<boolean[]> currentList = null;
		for (int i = 2; i <= 18; i++) {
			if (i % 3 == 0) {
				currentList = enhance(split(current)); //For iterations divisible by 3, a list output is more useful
			} else {
				current = i % 3 == 1 ? assemble(enhance(currentList)) : assemble(enhance(split(current)));
			}
		}
		//Find number of "on" cells
		int sum = 0;
		for (boolean b : assemble(currentList)) {
			if (b) sum++;
		}
